...
+ (MB_INSTANCETYPE)showText:(NSString *)text AddedTo:(UIView *)view animated:(BOOL)animated{
    MBProgressHUD *mbp = [[self alloc] initWithView:view];
    mbp.mode = MBProgressHUDModeText;
    mbp.detailsLabelText = text;
    [mbp show:YES];
    mbp.removeFromSuperViewOnHide = YES;
    [mbp hide:YES afterDelay:1.5];
    [view addSubview:mbp];
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:mbp action:@selector(closeSelf:)];
    [mbp addGestureRecognizer:tap];
    return mbp;
}

- (void)closeSelf:(UITapGestureRecognizer *)tap{
    [(MBProgressHUD *)tap.view hide:YES];
}
...
//        if (!CGRectEqualToRect([UIApplication appDelegate].keyboardRect, CGRectZero)) {
//            float y = [UIApplication appDelegate].keyboardRect.origin.y;
